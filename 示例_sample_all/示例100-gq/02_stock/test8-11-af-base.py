...
def _write_annual_rate(sheetin, row_idx, col_idx, annual_rate_text, styles, is_yellow=False):
    ...
    if is_yellow:
        if annual_val >= 4.5:
            sheetin.write(row_idx, col_idx, annual_rate_text, styles["yellow_far_pink"])
        elif annual_val <= 1.5:
            sheetin.write(row_idx, col_idx, annual_rate_text, styles["yellow_far_red"])
        else:
            sheetin.write(row_idx, col_idx, annual_rate_text, styles["yellow_far_normal"])
    else:
        if annual_val >= 4.5:
            sheetin.write(row_idx, col_idx, annual_rate_text, styles["far_pink"])
        elif annual_val <= 1.5:
            sheetin.write(row_idx, col_idx, annual_rate_text, styles["far_red"])
        else:
            sheetin.write(row_idx, col_idx, annual_rate_text, styles["far_normal"])
...
def create_styles():
    ...
    strategy_right.alignment = align_left
    styles["strategy"] = strategy_right


    styles["yellow_strategy"] = xlwt.XFStyle()
    styles["yellow_strategy"].font = normal_font
    styles["yellow_strategy"].alignment = align_left
    ...
    total_percent_right.font = normal_font
    ...
    styles["yellow_total_percent"].font = normal_font
...
def write_sheet_data2(
    ...
    for item in data_list:
        ...
        if rank == 10:
            ...
            sheetin.write(row_idx, 1, info["名称"], styles["yellow_base_text"])
            ...
            sheetin.write(row_idx, 1, info["名称"], styles["base_text"])
...
def write_sheet_data1(
    ...
    for item in data_list:
        ...
        if rank in (10, 20):
            sheetin.write(row_idx, 0, code, styles["yellow_base_digit"])
            sheetin.write(row_idx, 1, info["名称"], styles["base_yellow_text"])
            sheetin.write(row_idx, 2, info["总数量"], styles["yellow_base_digit"])
            sheetin.write(row_idx, 3, info["当前价"], styles["yellow_base_digit"])
            sheetin.write(row_idx, 4, info["金额"], styles["yellow_base_digit"])
            sheetin.write(row_idx, 5, info["仓位%"], styles["yellow_percent"])
            ...
            if is_stock_dividend_sheet:  # 分红股
                ...
                sheetin.write(row_idx, 10, next_report, styles["yellow_base_digit"])   # ??
                sheetin.write(row_idx, 11, div_date, styles["yellow_base_digit"])   # ??
                ...
                sheetin.write(row_idx, 10, last_date, styles["yellow_date_right"])   # ??
                sheetin.write(row_idx, 11, next_date, styles["yellow_date_right"])   # ??
                sheetin.write(row_idx, 12, per_div, styles["yellow_date_right"])
            ...
            if is_stock_dividend_sheet:  # 分红股
                ...
                sheetin.write(row_idx, 10, next_report, styles["base_digit"])   # ?
                sheetin.write(row_idx, 11, div_date, styles["base_digit"])   # ?
                ...
                sheetin.write(row_idx, 10, last_date, styles["date_right"])  # ?
                sheetin.write(row_idx, 11, next_date, styles["date_right"])  # ?
                sheetin.write(row_idx, 12, per_div, styles["date_right"])
