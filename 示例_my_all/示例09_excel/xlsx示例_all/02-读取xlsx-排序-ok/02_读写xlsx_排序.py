...
# 1. 读取班级原始成绩（假设列：姓名、语文、数学、英语）
df = pd.read_excel("班级成绩.xlsx", sheet_name="原始成绩")

# 2. 用 pandas 自动算分、排名（1行代码搞定复杂计算）
df["总分"] = df[["语文", "数学", "英语"]].sum(axis=1)  # 算每个人总分
df["班级排名"] = df["总分"].rank(method="min", ascending=False).astype(int)  # 按总分降序排名
df = df[["姓名", "语文", "数学", "英语", "总分", "班级排名"]]  # 调整列的显示顺序

# 3. 把统计好的成绩写入临时文件（给后续美化用）
df.to_excel("班级成绩_已排序.xlsx", sheet_name="成绩排名", index=False, engine="openpyxl")
...
#
# # 1. 读取 pandas 算好的成绩表
# wb = load_workbook("班级成绩_待美化.xlsx")
# ws = wb["成绩排名"]
#
# # 2. 定义美化样式（像设计成绩单模板）
