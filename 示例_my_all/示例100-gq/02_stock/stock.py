...
strategy_dict = {
    ...
    "128124": "可转债", "002762": "业绩反转", "501225": "套利基",
...
def get_combined_stock_sort_key(item):
    """
    合并股票sheet排序规则：
    1. 先按策略优先级：分红股 > 业绩反转 > 小盘猛牛 > 热点发展 > 涨停回调
    2. 同策略内按各自的日期规则排序
    """
...
# ---------------------- 15. 新增：合并5个股票策略数据 ----------------------
# 定义需要合并的股票策略列表
stock_strategies = ["分红股", "业绩反转", "小盘猛牛", "热点发展", "涨停回调"]
# 合并数据
combined_stock_data = []
